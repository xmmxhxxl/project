...
class MyMainWindow(QWidget, Ui_Form):
    ...
    def __init__(self):
        ...
        self.thread.sinout.connect(self.clickedsin)
        self.thread.start()

    def clickedsin(self, showImage):
    ...
    #                 print("方差是：", variance)
    #                 print("平均数是：", average)
    #                 if variance > 1000 and average > 500:
    #                     j += 1  #
    #                     cv.imwrite("../picture/image{}.png".format(i), self.image)
    #                     print("成功保存截图！！", j)
    #                 elif variance < 1000 and average < 500 and j >= 3:
    #                     # 使用多线程执行识别模块，节约代码执行时间
    #                     print("开始识别")
    #                     # th = Thread(target=self.screenshot)
    #                     # th.start()
    #                     # self.screenshot()
    #                     self.fi = fication()
    #                     self.fi.detect()
    ...
    #                     self.j = 0
    #                     if self.fi.xiangsidu >= 40:
    #                         self.totalPrice = 0
    #                         # 设置表格显示标签、相似度、价格
    #                         tabl.setItem(self.j, 0, QTableWidgetItem(str(self.fi.label2)))
    #                         tabl.setItem(self.j, 1, QTableWidgetItem(str(self.fi.price)))
            ...
            while True:
                ...
                if n % (self.frame * self.times) == 0:
                    ...
                    if variance > 1000 and average > 500:
                        ...
                        j = 0  # 清空标志
            self.sinout.emit(self.showImage)
        except Exception as ex:
            print("错误！！！\n" + str(ex))

    # def run(self):
    #     self.sinout.emit(self.showcream())
